...
登入雲端帳號 → 一鍵加密上傳；換電腦登入同帳號 → 一鍵下載還原。

- 後端 Supabase（免費 tier），整包資料 **AES-GCM 加密後才上傳**，雲端只存密文
- 加解密金鑰 = 雲端帳號密碼（**忘記密碼 = 雲端那份解不開**，刻意設計）
- 雲端帳號與本機鎖屏密碼是兩套獨立系統
- **非即時同步**：手動按「上傳/下載」，個人單機使用足夠
- 詳細架構與 Supabase 設定見 [CLAUDE.md 第 20 章](./CLAUDE.md#20-雲端備份跨電腦跨教室)
